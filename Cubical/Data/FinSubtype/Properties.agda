...
module Cubical.Data.FinSubtype.Properties where
...
open import Cubical.Data.FinSubtype.Base as Fin
...
private
  ...
  @0 expand×Emb : ∀ k → isEmbedding (expand× {k})
...
-- natural.
@0 isPropResidue : ∀ k n → isProp (Residue k n)
...
@0 Residue≡ : ∀ k n → Residue k n ≡ Residue k (k + n)
...
module Reduce (k₀ : ℕ) where
  ...
  @0 step : ∀ n → Residue k n → Residue k (k + n)
  step n = transport (Residue≡ k n)

  step′ : ∀ n → Residue k n → Residue k (k + n)
  step′ n = Residue≃ k n .fst

  step-link : step ≡ step′
  step-link = funExt₂ (λ n r → transportRefl _)

  @0 reduce : ∀ n → Residue k n
...
open Reduce using (reduce; reduce′; reduce≡) public
...
@0 extract≡ : ∀ k n → extract (reduce k n) ≡ extract (reduce k (suc k + n))
extract≡ k n
  = lemma₅ (suc k) n (reduce k n) ∙ cong extract (Reduce.reduce≡ k n)

@0 isContrResidue : ∀{k n} → isContr (Residue (suc k) n)
...
@0 ≤-·sk-cancel : ∀ {m} {k} {n} → m · suc k ≤ n · suc k → m ≤ n
...
@0 Fin+≡Fin⊎Fin : (m n : ℕ) → Fin (m + n) ≡ Fin m ⊎ Fin n
Fin+≡Fin⊎Fin m n = isoToPath (Fin+≅Fin⊎Fin m n)

-- Equivalence between FinI and Fin

sucFin : {N : ℕ} → Fin N → Fin (suc N)
sucFin (k , n , p) = suc k , n , (+-suc _ _ ∙ cong suc p)

FinI→Fin : (N : ℕ) → FinI N → Fin N
FinI→Fin zero ()
FinI→Fin (suc N) zero = 0 , suc-≤-suc zero-≤
FinI→Fin (suc N) (suc k) = sucFin (FinI→Fin N k)

Fin→FinI : (N : ℕ) → Fin N → FinI N
Fin→FinI zero (k , n , p) = Empty.rec (snotz (sym (+-suc n k) ∙ p))
Fin→FinI (suc N) (0 , n , p) = zero
Fin→FinI (suc N) ((suc k) , n , p) = suc (Fin→FinI N (k , n , p')) where
  p' : n + suc k ≡ N
  p' = injSuc (sym (+-suc n (suc k)) ∙ p)

secFin : (n : ℕ) → section (FinI→Fin n) (Fin→FinI n)
...
retFin : (n : ℕ) → retract (FinI→Fin n) (Fin→FinI n)
retFin 0 ()
retFin (suc N) zero = refl
retFin (suc N) (suc k) = cong FinI.suc (cong (Fin→FinI N) (Fin-fst-≡ refl) ∙ retFin N k)

FinIIsoFin : (N : ℕ) → Iso (FinI N) (Fin N)
Iso.fun (FinIIsoFin N) = FinI→Fin N
Iso.inv (FinIIsoFin N) = Fin→FinI N
Iso.rightInv (FinIIsoFin N) = secFin N
Iso.leftInv (FinIIsoFin N) = retFin N

FinI≃Fin : (N : ℕ) → FinI N ≃ Fin N
FinI≃Fin N = isoToEquiv (FinIIsoFin N)

@0 FinI≡Fin : (N : ℕ) → FinI N ≡ Fin N
FinI≡Fin N = ua (FinI≃Fin N)
