...
mutual
  ...
  data SubstRelDesc : ∀ {ℓA ℓ≅A ℓB}
    ...
    @0 el : ∀ {ℓA ℓ≅A ℓU} {A : Type ℓA} {𝒮-A : UARel A ℓ≅A}
...
private
  module Example (A : Type) (a₀ : A) where
    ...
    @0 example0 : DUARel (autoUARel Type) (λ X → X → A × X) ℓ-zero
    example0 = autoDUARel _ _

    @0 example0' : {X Y : Type} (e : X ≃ Y)
         (f : X → A × X) (g : Y → A × Y)
    ...
    @0 example1 : DUARel (autoUARel (Type × Type)) (λ (X , Z) → X → Z) ℓ-zero
    example1 = autoDUARel _ _

    @0 example1' : {X Y : Type} (e : X ≃ Y) {Z W : Type} (h : Z ≃ W)
         (f : X → Z) (g : Y → W)
    ...
    @0 example2 : DUARel (autoUARel (A → Type)) (λ B → B a₀) ℓ-zero
    example2 = autoDUARel _ _

    @0 example2' : {B C : A → Type} (e : (a : A) → B a ≃ C a)
         (b : B a₀) (c : C a₀)
         → e a₀ .fst b ≡ c
