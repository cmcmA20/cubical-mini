...
makeIsCommMonoid : {M : Type ℓ} {ε : M} {_·_ : M → M → M}
                   ...
                   (·Comm    : (x y : M) → x · y ≡ y · x)
                 → IsCommMonoid ε _·_
IsCommMonoid.isMonoid (makeIsCommMonoid ·Assoc ·IdR ·Comm) =
  makeIsMonoid ·Assoc ·IdR (λ x → ·Comm _ _ ∙ ·IdR x)
IsCommMonoid.·Comm (makeIsCommMonoid ·Assoc ·IdR ·Comm) = ·Comm
...
makeCommMonoid : {M : Type ℓ} (ε : M) (_·_ : M → M → M)
                 ...
                 (·Comm    : (x y : M) → x · y ≡ y · x)
               → CommMonoid ℓ
fst (makeCommMonoid ε _·_ ·Assoc ·IdR ·Comm) = _
CommMonoidStr.ε (snd (makeCommMonoid ε _·_ ·Assoc ·IdR ·Comm)) = ε
CommMonoidStr._·_ (snd (makeCommMonoid ε _·_ ·Assoc ·IdR ·Comm)) = _·_
CommMonoidStr.isCommMonoid (snd (makeCommMonoid ε _·_ ·Assoc ·IdR ·Comm)) =
  makeIsCommMonoid ·Assoc ·IdR ·Comm
...
instance
  IsSetFromIsCommMonoid : {M : Type ℓ} {ε : M} {_·_ : M → M → M}
                          {isCommMonoid : IsCommMonoid ε _·_}
                        → IsSet M
  IsOfHLevel.iohl (IsSetFromIsCommMonoid {isCommMonoid = isCommMonoid}) =
    let open IsCommMonoid isCommMonoid in is-set .iohl

  IsSetCommMonoid : {M : CommMonoid ℓ} → IsSet ⟨ M ⟩
  IsSetCommMonoid {M = M} =
    let open CommMonoidStr (snd M)
    in IsSetFromIsCommMonoid {isCommMonoid = isCommMonoid}
...
isPropIsCommMonoid ε _·_  =
  isOfHLevelRetractFromIso 1 IsCommMonoidIsoΣ
    (isPropΣ (isPropIsMonoid ε _·_)
             λ mon → isPropΠ2 (λ _ _ → mon .is-set .iohl _ _))
  where
  open IsMonoid

@0 𝒮ᴰ-CommMonoid : DUARel (𝒮-Univ ℓ) CommMonoidStr ℓ
𝒮ᴰ-CommMonoid =
...
@0 CommMonoidPath : (M N : CommMonoid ℓ) → CommMonoidEquiv M N ≃ (M ≡ N)
