...
@0 decodeEncode : (x : S¹) (p : base ≡ x) → decode x (encode x p) ≡ p
...
@0 windingℤLoop : (n : ℤ) → winding (intLoop n) ≡ n
...
@0 winding-hom : (a b : ΩS¹) → winding (a ∙ b) ≡ (winding a) + (winding b)
winding-hom a b i =
...
private
  ...
  @0 loop-conjugation : basedΩS¹→ΩS¹ i1 ≡ λ x → x
  ...
  @0 basedΩS¹→ΩS¹≡basechange : (i : I) → basedΩS¹→ΩS¹ i ≡ basechange (loop i)
  ...
  @0 basechange-isequiv-aux : (i : I) → isEquiv (basechange (loop i))
  ...
  @0 basechange-isequiv : (x : S¹) → isEquiv (basechange x)
  ...
  @0 basedΩS¹≡ΩS¹' : (x : S¹) → basedΩS¹ x ≡ ΩS¹
  basedΩS¹≡ΩS¹' x = ua (basechange x , basechange-isequiv x)

  @0 basedΩS¹≡ℤ' : (x : S¹) → basedΩS¹ x ≡ ℤ
...
@0 isSetΩx : (x : S¹) → isSet (x ≡ x)
isSetΩx = toPropElim (λ _ → isPropIsSet) isSetΩS¹

@0 basechange2 : (x : S¹) → ΩS¹ → (x ≡ x)
...
@0 basechange2-sect : (x : S¹) → section (basechange2 x) (basechange2⁻ x)
basechange2-sect =
  toPropElim (λ _ → isOfHLevelΠ 1 λ _ → isSetΩx _ _ _ )
             λ _ → refl

@0 basechange2-retr : (x : S¹) → retract (basechange2 x) (basechange2⁻ x)
basechange2-retr =
  toPropElim (λ s → isOfHLevelΠ 1 λ x → isSetΩx _ _ _)
             λ _ → refl

@0 Iso-basedΩS¹-ΩS¹ : (x : S¹) → Iso (basedΩS¹ x) ΩS¹
...
@0 Iso-basedΩS¹-ℤ : (x : S¹) → Iso (basedΩS¹ x) ℤ
...
@0 basechange2⁻-morph : (x : S¹) (p q : x ≡ x) →
