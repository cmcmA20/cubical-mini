...
whisker-path-lₑ ac = iso→≃ $ sym ac ∙_ , iso (ac ∙_) (λ _ → ∙-cancel-l _ _) (λ _ → ∙-cancel-l _ _)

whisker-path-rₑ : (b ＝ c) → (a ＝ b) ≃ (a ＝ c)
whisker-path-rₑ bc = iso→≃ $ _∙ bc , iso (_∙ sym bc) (λ _ → ∙-cancel-r _ _) (λ _ → ∙-cancel-r _ _)

whisker-lₑ : (A ≃ C) → (A ≃ B) ≃ (C ≃ B)
whisker-lₑ ac = iso→≃ $ ac ₑ⁻¹ ∙ₑ_ , iso (ac ∙ₑ_) (λ _ → ∙ₑ-cancel-l _ _) λ ab →
  ∙ₑ-assoc _ _ _ ∙ ap (_∙ₑ ab) (∙ₑ-inv-r _) ∙ ∙ₑ-id-l _

whisker-rₑ : (B ≃ C) → (A ≃ B) ≃ (A ≃ C)
whisker-rₑ bc = iso→≃ $ _∙ₑ bc , iso (_∙ₑ bc ₑ⁻¹) (λ _ → ∙ₑ-cancel-r _ _) λ ab →
