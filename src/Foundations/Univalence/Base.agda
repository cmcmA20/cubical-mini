...
path→Equiv-refl {A} = Σ-PathP (λ i x → coe1→i (λ i → A) i x)
                              (to-PathP (is-prop→PathP (λ i → is-equiv-is-prop _) _ _))
...
Path≃Equiv = path→Equiv , r where
  r : is-iso path→Equiv
  r .is-iso.inv = ua
  r .is-iso.rinv (f , is-eqv) = Σ-PathP (fun-ext (ua-β (f , is-eqv)))
                                        (to-PathP (is-equiv-is-prop f _ _))
