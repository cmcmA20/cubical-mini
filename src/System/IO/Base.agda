...
instance
  mutual
    IO-map : Map (eff IO)
    IO-map .Map._<$>_ = λ f mx → do
      x ← mx
      pure (f x)

    IO-idiom : Idiom (eff IO)
    IO-idiom .Idiom.Map-idiom = IO-map
    IO-idiom .Idiom.pure = return
    IO-idiom .Idiom._<*>_ = λ mf mx → do
      f ← mf
      x ← mx
      pure (f x)

    IO-bind : Bind (eff IO)
    IO-bind .Bind._>>=_ = _>>=′_
    IO-bind .Bind.Idiom-bind = IO-idiom
