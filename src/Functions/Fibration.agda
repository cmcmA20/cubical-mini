...
Σ-fibre-equiv : (B : A → Type ℓᵇ) (a : A)
              → fibre fst a ≃ B a
Σ-fibre-equiv B a = iso→≃ isom where
  isom : _ ≅ _
...
module @0 _ where
  opaque
    ...
    fibration-equiv : {B : Type ℓᵇ} {ℓ : Level}
                    → Σ[ E ꞉ Type (ℓᵇ ⊔ ℓ) ] (E → B)
                    ≃ (B → Type (ℓᵇ ⊔ ℓ))
    fibration-equiv {B} {ℓ} = iso→≃ isom where
...
fibre-comp {g} {f} {c} = iso→≃ $ to , iso from ri li where
