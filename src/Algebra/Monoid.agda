...
opaque
  ...
  is-monoid-is-prop M M′ = Equiv.injective (isoₜ→equiv is-monoid-iso) $
...
monoid-on↪semigroup-on .snd = set-injective→is-embedding hlevel! λ p →
  Equiv.injective (isoₜ→equiv monoid-on-iso) $ ap Semigroup-on._⋆_ p ,ₚ prop!
...
monoid-on↪unital-magma-on .snd = set-injective→is-embedding hlevel! λ {x} {y} p →
  Equiv.injective (isoₜ→equiv monoid-on-iso) $ ap UMagma-on._⋆_ p ,ₚ prop!
